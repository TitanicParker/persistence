...
const rightsText = [
  copyrightLine, '', ...rightsLead, '', 'LIMITED PERSONAL-STUDY PERMISSION', ...personalStudyPermission,
  '', 'PERMISSION ENQUIRIES FOR BROADER USE', ...permissionInvitation,
  '', 'PUBLIC ACCESS AND REUSE', ...publicAccessAndReuse,
  '', 'ACKNOWLEDGEMENT', ...acknowledgementText,
].join('\n\n');

const introduction = [
  'CONSTRAINT GRAMMAR OF COMPLETED INTELLIGIBILITY', 'PORTABLE KNOWLEDGE EDITION', '',
  `Publication version: ${publicationVersion}`, `Publication date: ${publicationDate}`,
  `Canonical publication: ${canonicalUrl}`, `Repository: ${repositoryUrl}`, '',
