...
.cg-age-path{font-family:ui-sans-serif,system-ui,sans-serif;background:#fffdf8;border-bottom:1px solid #e3ded4;padding:.72rem max(1rem,calc((100vw - 1180px)/2));position:sticky;top:0;z-index:15}.cg-age-path__inner{display:flex;align-items:center;gap:.8rem}.cg-age-path__label{flex:0 0 auto;font-size:.7rem;letter-spacing:.1em;text-transform:uppercase;color:#68645d;font-weight:700}.cg-age-path__rail{display:flex;gap:.38rem;overflow-x:auto;scrollbar-width:none;overscroll-behavior-x:contain;padding:.12rem .05rem}.cg-age-path__rail::-webkit-scrollbar{display:none}.cg-age-path a{flex:0 0 auto;display:inline-flex;align-items:center;justify-content:center;min-width:2.25rem;height:2.25rem;padding:0 .65rem;border:1px solid #d4cec3;border-radius:999px;color:#504c46;text-decoration:none;font-size:.78rem;font-weight:700}.cg-age-path a:hover{background:#f1ede4}.cg-age-path a:focus-visible{outline:3px solid #2f5d50;outline-offset:2px}.cg-age-path a[aria-current="page"]{background:#1c1c19;color:#fffdf8;border-color:#1c1c19}.cg-age-path__hint{flex:0 0 auto;color:#777168;font-size:.72rem;white-space:nowrap}
...
body[data-developmental-reader]{transition:opacity .12s ease,transform .12s ease}body[data-developmental-reader].is-leaving-next{opacity:.72;transform:translateX(-8px)}body[data-developmental-reader].is-leaving-previous{opacity:.72;transform:translateX(8px)}
...
@media(prefers-reduced-motion:reduce){body[data-developmental-reader]{transition:none}.cg-developmental-gateway__link{transition:none}}
</style>`;
...
const developmentalGateway = `<section class="cg-developmental-gateway" aria-labelledby="cg-developmental-gateway-title"><p class="cg-developmental-gateway__eyebrow">There is another step</p><h2 id="cg-developmental-gateway-title">Keep going.</h2><p>You do not need to leave this explanation behind. The same ideas can become a little more exact while everything you have understood so far stays with you.</p><a class="cg-developmental-gateway__link" href="/persistence/five-year-old/6/#essay-start">Continue the explanation <span aria-hidden="true">→</span></a></section>`;

function agePath(current: number): string {
  const links = Array.from({ length: 14 }, (_, index) => index + 5).map((age) => {
    const href = age === 5 ? '/persistence/five-year-old/#essay-start' : `/persistence/five-year-old/${age}/#essay-start`;
    const currentAttribute = age === current ? ' aria-current="page" data-current-age' : '';
    return `<a href="${href}"${currentAttribute} aria-label="${age}-Year-Old Edition">${age}</a>`;
  }).join('');
...
for (const edition of editions) {
  ...
  if (edition.route === 'five-year-old/') {
    ...
    html = html.replace('<main>', '<main id="essay-start">');
